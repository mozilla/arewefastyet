...
function solveTest(level, after) {
  ...
  tests.push(function() {
    ...
    //XXXdocument.getElementById("timeFinished").textContent = timeElapsed.toFixed(3) + "s";
  });
  tests.push(after);
  ...
  var timeoutFunc = function() {
    if(current < tests.length) {
      tests[current]();
      current++;
      timeoutFunc(); //XXXwindow.setTimeout(timeoutFunc, 300);
    }
  }
...
function solveTestHelper(puzzle, iterations) {
  ...
  var end = new Date();
  //XXXrenderBoard(puzzle);
  //XXXrenderSolvedBoard(solution);
...
function initialize() {
  ...
  //XXXrenderBoard(currentPuzzle);

  /*XXXvar amazeButton = document.getElementById('amazeButton');
  ...
  //XXXamazeButton.addEventListener('click', 
  amazeFun = function() {
    if(!amazing) {
	var level = 2; // 5000 games, default XXX parseInt(difficulty.options[difficulty.selectedIndex].value);
      amazing = true;
      //XXXclearErrors();
      //XXXfinishedCalculatingDiv.style.display = 'none';
      //XXXcalculatingDiv.style.display = 'block';
      solveTest(level, function() {
        /*XXXfinishedCalculatingDiv.style.display = 'block';
